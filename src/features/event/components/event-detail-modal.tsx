...
const REVIEWS = [
  {
    id: 1,
    user: { name: "Sarah J.", avatar: "https://i.pravatar.cc/150?u=1" },
    rating: 5,
    date: "2 days ago",
    text: "This event was absolutely amazing! The vibe was electric and well organized.",
  },
  {
    id: 2,
    user: { name: "Mike T.", avatar: "https://i.pravatar.cc/150?u=2" },
    rating: 4,
    date: "1 week ago",
    text: "Great experience overall. The venue was easy to find, but parking was a bit tight.",
  },
  {
    id: 3,
    user: { name: "Emily R.", avatar: "https://i.pravatar.cc/150?u=3" },
    rating: 5,
    date: "2 weeks ago",
    text: "Had a blast with my friends! Highly recommended for anyone interested.",
  },
  {
    id: 4,
    user: { name: "David K.", avatar: "https://i.pravatar.cc/150?u=4" },
    rating: 3,
    date: "3 weeks ago",
    text: "It was okay. Expected a bit more content for the price, but still fun.",
  },
];
...
export function EventDetailModal() {
  ...
  const event = activeEventId
    ? SPACE_EVENTS.find((e) => e.id === activeEventId)
    : null;
    ...
    <AnimatePresence>
      {activeEventId && (
        <motion.div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setActiveEventId(null);
          }}
        >
          <motion.div
                  ...
                  <div className="flex items-start gap-3.5">
                    <div className="size-11 rounded-xl bg-muted/60 border border-border flex items-center justify-center shrink-0">
                      {isOnline ? (
                        ...
                        <MapPoint className="size-4.5 text-muted-foreground" />
                      )}
                    </div>
                ...
                <div className="lg:w-64 shrink-0 space-y-4">
                  ...
                  {isUpcoming && (
                    <div className="rounded-2xl bg-muted/40 p-4 space-y-2.5">
                      ...
                      <p className="text-[13px] text-foreground/70">
                        Bergabung dan jangan sampai ketinggalan!
                      </p>
                  ...
                  <div>
                    ...
                    <div className="flex -space-x-2">
                      {Array.from({
                        length: Math.min(6, event.attendees_count),
                      }).map((_, i) => (
                        <img
                          key={i}
              ...
              <div className="flex items-center gap-6 border-b border-border mb-6">
                {(["about", "reviews"] as const).map((tab) => (
                  <button
                    ...
                    className={`relative pb-3 text-[14px] font-semibold capitalize transition-colors ${
                      activeTab === tab
                        ? "text-foreground"
                        ...
                        <div className="text-center space-y-1">
                          ...
                          <div className="flex gap-0.5 justify-center text-yellow-500">
                            {[1, 2, 3, 4, 5].map((s) => (
                              <Star
                                key={s}
                                className={`size-3.5 ${s <= Math.round(RATING_SUMMARY.average) ? "fill-current" : "text-muted-foreground/30"}`}
                              />
                            ))}
                          </div>
                          ...
                          {[5, 4, 3, 2, 1].map((rating) => {
                            const count =
                              RATING_SUMMARY.distribution[
                                rating as keyof typeof RATING_SUMMARY.distribution
                              ];
                            const percent =
                              (count / RATING_SUMMARY.total) * 100;
                            return (
                              <div
                                key={rating}
                                className="flex items-center gap-3"
                              >
                                <span className="text-[12px] font-medium w-3 text-muted-foreground">
                                  {rating}
                                </span>
                            ...
                            <div className="space-y-1.5 flex-1">
                              ...
                              <div className="flex gap-0.5 text-yellow-500">
                                {[1, 2, 3, 4, 5].map((s) => (
                                  <Star
                                    key={s}
                                    className={`size-3 ${s <= review.rating ? "fill-current" : "text-muted-foreground/20"}`}
                                  />
                                ))}
                              </div>
