...
import { UserCheck, Bookmark, FireMinimalistic } from '@solar-icons/react';
...
export function SpaceSidebar() {
   ...
   const FEED_ITEMS = [
      ...
      { id: 'following', href: '/following', label: 'Following', icon: UserCheck },
   ...
   return (
      <div className="hidden lg:flex flex-col w-52 sticky top-24 pr-2 h-[calc(100vh-120px)] overflow-y-auto no-scrollbar shrink-0 gap-8">
            ...
            <div className="flex flex-col">
               {FEED_ITEMS.map((item) => {
                  ...
                  return (
                     ...
                     >
                        <item.icon className="size-6 text-foreground" weight={isActive ? "Bold" : "Linear"} />
                        <span className={cn("text-[17px] text-foreground", isActive ? "font-bold" : "font-normal")}>
                           {item.label}
                        </span>
                     </Link>
                  );
         ...
         {/* Followed Spaces */}
         <div>
            <h3 className="font-bold text-[15px] px-2 mb-3 text-foreground/80">Your Spaces</h3>
                  ...
                  return (
                     ...
                     >
                        ...
                        <div className="flex-1 min-w-0">
                           <p className="text-[11px] text-muted-foreground truncate">s/{space.slug}</p>
                           <h4 className={cn("font-semibold text-[14px] truncate", isActive ? "text-foreground" : "text-muted-foreground group-hover:text-foreground")}>
                              {space.name}
                           </h4>
                        </div>
                     </Link>
         ...
         <div className="mt-2 px-2">
            <Link href="/discover" className="text-xs text-muted-foreground hover:underline cursor-pointer">
