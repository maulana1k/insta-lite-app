...
import { ChevronRight, ChevronLeft, X, ArrowLeftRight, Check } from 'lucide-react';
...
const MOCK_FOLLOW_REQUESTS: FollowRequest[] = [
  { id: 'fr1', username: 'kaidev', full_name: 'Kai Nakamura', avatar_url: 'https://i.pravatar.cc/150?u=kai', mutual_follower: 'budikoding', date: '2025-02-13' },
  { id: 'fr2', username: 'liambuilds', full_name: "Liam O'Brien", avatar_url: 'https://i.pravatar.cc/150?u=liam', date: '2025-02-12' },
  { id: 'fr3', username: 'miathemaker', full_name: 'Mia Torres', avatar_url: 'https://i.pravatar.cc/150?u=mia', mutual_follower: 'dinda.ui', date: '2025-02-11' },
  { id: 'fr4', username: 'ryanstartup', full_name: 'Ryan Hughes', avatar_url: 'https://i.pravatar.cc/150?u=ryan', date: '2025-02-10' },
  { id: 'fr5', username: 'jakethesnake', full_name: 'Jake Williams', avatar_url: 'https://i.pravatar.cc/150?u=jake', mutual_follower: 'sarahcodes', date: '2025-02-09' },
];
...
const MOCK_NOTIFICATIONS: Notification[] = [
  ...
  },
  {
    ...
    message: 'menyukai postingan Anda.',
    date: '2025-02-12',
    thumbnail_url: 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=80&h=80&fit=crop',
  },
  {
    id: 'n5',
    type: 'follow',
                ...
                <AnimatePresence initial={false} mode="popLayout">
                  {view === 'notifications' ? (
                    <motion.div
                      ...
                      initial={slideDirection === 'back' ? 'enterFromLeft' : 'center'}
                      animate="center"
                      exit="exitToLeft"
    ...
    <>
      ...
      <div className="overflow-y-auto overscroll-contain flex-1">
        ...
        {activeFilter === 'all' && (
          ...
          >
            <span className="text-[14px] font-semibold">Permintaan mengikuti</span>
            <div className="flex items-center gap-2">
              <span className="size-2 rounded-full bg-red-500" />
              <span className="text-[14px] text-muted-foreground">{MOCK_FOLLOW_REQUESTS.length}</span>
        ...
        {newNotifs.length > 0 && (
          <>
            <div className="px-5 pt-3 pb-1.5">
              <span className="text-[13px] font-semibold text-muted-foreground">Baru</span>
            </div>
            {newNotifs.map((notif) => (
        ...
        {olderNotifs.length > 0 && (
          <>
            <div className="px-5 pt-4 pb-1.5">
              <span className="text-[13px] font-semibold text-muted-foreground">Sebelumnya</span>
            </div>
        ...
        {filtered.length === 0 && (
          <div className="px-5 py-10 text-center">
            <p className="text-[14px] text-muted-foreground">Belum ada notifikasi.</p>
          </div>
        )}
    ...
    <>
      ...
      <div className="overflow-y-auto overscroll-contain flex-1">
        ...
        {MOCK_FOLLOW_REQUESTS.length === 0 && (
          <div className="px-5 py-10 text-center">
            <p className="text-[14px] text-muted-foreground">Tidak ada permintaan mengikuti.</p>
          </div>
        )}
      ...
      <div className="flex items-center gap-3 px-5 py-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[14px] text-muted-foreground">
            <span className="font-semibold text-foreground/60">{request.username}</span>{' '}
            diterima
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3 hover:bg-muted/40 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[12px] text-muted-foreground truncate">{request.full_name}</p>
...
function NotificationItem({ notif }: { notif: Notification }) {
      ...
      <button className="w-full flex items-center gap-3 px-5 py-3 hover:bg-muted/40 transition-colors text-left">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            ...
            {notif.systemDot && <span className="size-1.5 rounded-full bg-red-500" />}
          </div>
          <p className="text-[13px] text-muted-foreground truncate">{notif.message}</p>
        </div>
  ...
  return (
    <button className="w-full flex items-center gap-3 px-5 py-3 hover:bg-muted/40 transition-colors text-left">
      <div className="size-12 rounded-full overflow-hidden shrink-0">
        <img src={notif.avatar_url} alt={notif.username} className="w-full h-full object-cover" />
      </div>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[12px] text-muted-foreground mt-0.5">{formatDate(notif.date)}</p>
      </div>
      ...
      {notif.thumbnail_url && (
        <div className="size-11 rounded-lg overflow-hidden shrink-0">
          <img src={notif.thumbnail_url} alt="" className="w-full h-full object-cover" />
        </div>
      )}
    </button>
  );
