import { Plain, ChatRound, VerifiedCheck } from '@solar-icons/react';
import { ArrowDown, AudioWaveform, CornerDownRight, Ellipsis, EllipsisVertical, EyeOff, Heart, RefreshCcw, Share } from 'lucide-react'
                  ...
                  {post.space ? (
                     <>
                        <div className="absolute top-0 left-0 w-7 h-7 rounded-md overflow-hidden border border-border/50">
                           <img src={post.space.avatar_url} alt={post.space.name} className="w-full h-full object-cover" />
                        </div>
                        {post.is_anonymous ? (
            ...
            <div className="flex-1 min-w-0 pb-2">
               ...
               <div className="flex items-center justify-between -mt-1">
                  <div className="flex items-center gap-1.5 ">
                     {post.space && (
                        <>
                           <span className="font-semibold text-[15px] ">{post.space.name}</span>
                           <span className="text-[15px]">/</span>
                        </>
                     )}
                     {post.is_anonymous ? (
                        <span className="font-semibold text-[15px] text-muted-foreground">Member</span>
                     ) : (
                        <>
                           <span className="font-semibold text-[15px] leading-none">{post.user.username}</span>
               ...
               <div className="flex items-center gap-1 mt-1 -ml-2 pb-3">
                  ...
                  <ActionButton icon={ChatRound} count={post.comments_count} href={`/post/${post.id}`} />
            ...
            <div className="flex items-center gap-2 pb-2">
               ...
               */}
               ...
               <div className="shrink-0 w-10 flex justify-center">
                  ...
                  {showReplies ? (
                     <EllipsisVertical strokeWidth={3} className="size-5 text-muted-foreground/20" />
                  ) : (
                     <AudioWaveform strokeWidth={3} className="size-5 text-muted-foreground/20 rotate-90" />
                  )}
...
function EmbeddedRepost({ repost }: { repost: Post }) {
   const maxLen = 200;
   const truncated = repost.content.length > maxLen
      ...
      <div className="mb-3 rounded-2xl border border-border p-3 hover:bg-muted/30 transition-colors cursor-pointer">
         <div className="flex items-center gap-2 mb-2">
            <div className="w-5 h-5 rounded-full overflow-hidden shrink-0">
               <img src={repost.user.avatar_url} alt={repost.user.username} className="w-full h-full object-cover" />
            </div>
            <span className="font-semibold text-[13px]">{repost.user.username}</span>
            {repost.user.verified && <VerifiedCheck className="w-3.5 h-3.5 text-blue-500 fill-blue-500/10" />}
            {repost.space && (
               <span className="text-[12px] text-muted-foreground">in {repost.space.name}</span>
            )}
            <span className="text-[12px] text-muted-foreground">&middot; {timeAgo(repost.created_at)}</span>
         </div>
...
function ActionButton({ icon: Icon, count, href }: { icon: any; count?: number; href?: string }) {
   const content = (
      <div className="flex items-center gap-1.5 px-2 py-1.5 -my-1.5 rounded-full hover:bg-muted/80 transition-colors group cursor-pointer">
         ...
         {count !== undefined && count > 0 && (
            <span className="text-[13px] font-medium text-foreground/70 tabular-nums">{formatCount(count)}</span>
         )}
      </div>
   );

   if (href) {
      return <Link href={href} className="muted-foreground">{content}</Link>;
   }
...
function InlineComment({ comment, isReply = false }: { comment: PostComment; isReply?: boolean }) {
   const [showReplies, setShowReplies] = useState(false);

   return (
      <div className={`flex gap-2.5 ${isReply ? 'py-1.5' : 'py-2'} px-1`}>
         {/* Avatar */}
         <div className={`rounded-full overflow-hidden shrink-0 ${isReply ? 'w-6 h-6' : 'size-8'}`}>
         ...
         <div className="flex-1 min-w-0">
            <p className="text-[15px] leading-snug -mt-0.5">
               ...
               {comment.user.verified && <VerifiedCheck className="w-3 h-3 text-blue-500 inline ml-0.5 -mt-0.5" />}
               {' '}
